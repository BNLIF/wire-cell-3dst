...
int main(int argc, char* argv[])
{
  ...
  //TH1F *h_nois = new TH1F("h_nois","h_nois",1024*4,0,512*4);
  ...
  TF1 *filter_gauss = new TF1("gauss","exp(-[0]*x*x)",0,1);
  //filter_gauss->SetParameter(0,4.);
  filter_gauss->SetParameter(0,0.01);
  TF1 *filter_wiener = new TF1("wiener","exp(-[0]*pow(x,[1]))",0,1);
  ...
  for (Int_t i=0;i!=51;i++){
    ...
    //   sum[i] += h_decon->GetBinContent(i*20+j+1);
    // }
    // sum[i] *= 6241.;
  }
